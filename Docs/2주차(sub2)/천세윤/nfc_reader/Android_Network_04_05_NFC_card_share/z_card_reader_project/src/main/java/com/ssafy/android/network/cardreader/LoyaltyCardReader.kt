 ...
 */
private const val TAG = "LoyaltyCardReader_싸피"
class LoyaltyCardReader(accountCallback: AccountCallback) : ReaderCallback {
    ...
    override fun onTagDiscovered(tag: Tag) {
        ...
        if (isoDep != null) {
            try {
                ...
                i(TAG, "Requesting remote AID: " + SAMPLE_LOYALTY_CARD_AID)
                ...
                val bytepayload = String(payload, Charsets.UTF_8)

                i(TAG,"payload: $bytepayload")
                if (Arrays.equals(SELECT_OK_SW, statusWord)) {
                    // The remote NFC device will immediately respond with its stored account number

                    val accountNumber = String(payload, Charset.forName("UTF-8"))
                    i(TAG,"Received: $accountNumber")
    ...
    companion object {
        ...
        private const val SAMPLE_LOYALTY_CARD_AID = "D2760000850101"
